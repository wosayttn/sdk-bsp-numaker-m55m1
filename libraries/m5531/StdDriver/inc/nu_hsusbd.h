 ...
 * @file     nu_hsusbd.h
 * @version  V1.00
 * @brief    M5531 series HSUSBD driver header file
 *
...
typedef struct s_hsusbd_info
{
    ...
    const uint8_t *gu8ConfigDesc;         /*!< Config descriptor */
    const uint8_t **gu8StringDesc;        /*!< Pointer for USB String Descriptor pointers */
    const uint8_t *gu8QualDesc;           /*!< Qualifier descriptor */
    const uint8_t *gu8FullConfigDesc;     /*!< Full Speed Config descriptor */
    const uint8_t *gu8HSOtherConfigDesc;  /*!< Other Speed Config descriptor */
    const uint8_t *gu8FSOtherConfigDesc;  /*!< Other Speed Config descriptor */
    const uint8_t *gu8BosDesc;            /*!< Pointer for USB BOS Descriptor */
    const uint8_t **gu8HidReportDesc;     /*!< Pointer for HID Report descriptor */
...
/** @cond HIDDEN_SYMBOLS */
extern uint32_t volatile g_u32HsEpStallLock;
extern uint8_t volatile g_hsusbd_Configured;
extern uint8_t volatile g_hsusbd_ShortPacket;
extern uint8_t volatile g_hsusbd_CtrlZero;
extern uint8_t volatile g_hsusbd_UsbAddr;
extern uint32_t volatile g_hsusbd_DmaDone;
extern uint32_t volatile g_hsusbd_CtrlInSize;
