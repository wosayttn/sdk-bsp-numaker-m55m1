 ...
 * @file     hsusbd.c
 * @version  V1.00
 * @brief    M5531 series HSUSBD driver source file
 *
...
uint32_t volatile g_u32HsEpStallLock = 0ul;       /* Bit map flag to lock specified EP when SET_FEATURE */
...
volatile uint8_t g_u8GetDesDevice = 0ul;
volatile uint8_t g_hsusbd_RemoteWakeupEn = 0ul;

static uint8_t g_hsusbd_buf[12];

uint8_t volatile g_hsusbd_Configured = 0ul;
uint8_t volatile g_hsusbd_CtrlZero = 0ul;
uint8_t volatile g_hsusbd_UsbAddr = 0ul;
uint8_t volatile g_hsusbd_ShortPacket = 0ul;
uint32_t volatile g_hsusbd_DmaDone = 0ul;
uint32_t volatile g_hsusbd_CtrlInSize = 0ul;
