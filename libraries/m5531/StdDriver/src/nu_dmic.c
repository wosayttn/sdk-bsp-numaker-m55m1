...
/**
  ...
  */
uint32_t DMIC_SetSampleRateEx(DMIC_T *dmic, uint32_t u32SampleRate, uint32_t u32TargetCLK_Min, uint32_t u32TargetCLK_Max)
{
    uint16_t const au16OSRTable[] = {64, 128, 256, 100, 64, 64, 64, 50};
    uint32_t u32SourceClock, u32BusClock, u32MainClock, u32OSR, u32MDiv, u32MinDiff = ~1U;
    int i, n = -1, div = 1;
    uint32_t u32Ret = 0;
    ...
    u32SourceClock = u32SourceClock / (CLK->DMICDIV + 1);

    for (i = 0; i < sizeof(au16OSRTable) / sizeof(uint16_t); i++)
    {
        u32BusClock = u32SampleRate * au16OSRTable[i];

        if ((u32BusClock >= u32TargetCLK_Min) &&
                (u32BusClock <= u32TargetCLK_Max))
        {
            u32MDiv = u32SourceClock / u32BusClock;

            if (u32MDiv < 65)
            {
                uint32_t u32FindSR = u32SourceClock / u32MDiv / au16OSRTable[i];
                uint32_t u32Diff;

                if (u32SampleRate > u32FindSR)
                {
                    u32Diff = u32SampleRate - u32FindSR;
                }
                else
                {
                    u32Diff = u32FindSR - u32SampleRate;
                }

                if (u32MinDiff > u32Diff)
                {
                    u32MinDiff = u32Diff;
                    n = i;
                    div = u32MDiv;
    ...
    if (n != -1)
    {
        dmic->DIV = (dmic->DIV & ~DMIC_DIV_OSR_Msk) | (n << DMIC_DIV_OSR_Pos);
        dmic->DIV = (dmic->DIV & ~DMIC_DIV_DMCLKDIV_Msk) | ((div - 1) << DMIC_DIV_DMCLKDIV_Pos);

        u32Ret = DMIC_GetSampleRate(dmic);

#if 0
        printf("[%s] SrcClk: %d Hz, Wanted_BusClk: %d Hz, Real_BusClk: %d Hz, n: %d, OSR: %d, div: %d\n",
               __func__,
               u32SourceClock,
               u32SampleRate * au16OSRTable[n],
               u32Ret * au16OSRTable[n],
               n,
               au16OSRTable[n],
               div);
#endif
