...
static uint32_t HyperRAM_TrimDLLDelayNumber(SPIM_T *spim)
{
    uint8_t u8RdDelay = 0;
...
uint32_t HyperRAM_Init(SPIM_T *spim)
{
    uint32_t u32RdDelay;
    ...
    //rt_kprintf("Applied CSM_Time:%d  RdLatency:%d, WrLatency:%d\n", HYPERRAM_CSM_TIME, HYPERRAM_RD_LTCY, HYPERRAM_WR_LTCY);

    /* Reset HyperRAM */
    SPIM_HYPER_Reset(spim);

    /* Trim DLL component delay stop number */
    u32RdDelay = HyperRAM_TrimDLLDelayNumber(spim);
    if (CONFIG_SPIM_OPTIM)
    {
        uint32_t u32HRAMReg0 = SPIM_HYPER_ReadHyperRAMReg(spim, SPIM_HYPER_HRAM_CONFIG_REG0);

        //rt_kprintf("Before u32HRAMReg0:%08x\n", u32HRAMReg0);
        ...
        //rt_kprintf("Updated u32HRAMReg0:%08x\n", u32HRAMReg0);

        u32HRAMReg0 = SPIM_HYPER_ReadHyperRAMReg(spim, SPIM_HYPER_HRAM_CONFIG_REG0);

        //rt_kprintf("After u32HRAMReg0:%08x\n", u32HRAMReg0);
