...
static int wdt_pm_frequency_change(const struct rt_device *device, rt_uint8_t mode)
{
    ...
    if (clk == CLK_WDTSEL_WDT0SEL_HCLK2_DIV2048)
    {
...
static uint32_t wdt_get_module_clock(void)
{
    return (CLK_GetModuleClockSource(WDT0_MODULE) << CLK_WDTSEL_WDT0SEL_Pos);
}
...
static uint32_t wdt_get_working_hz(void)
{
    ...
    switch (clk)
    {
    case CLK_WDTSEL_WDT0SEL_LIRC:
        hz = __LIRC;
        break;

    case CLK_WDTSEL_WDT0SEL_LXT:
        hz = __LXT;
        break;
...
static rt_err_t wdt_control(rt_watchdog_t *dev, int cmd, void *args)
{
    ...
    case RT_DEVICE_CTRL_WDT_START:
        ...
        WDT_Open(WDT0, MIN_TOUTSEL, WDT_RESET_DELAY_1026CLK, TRUE, TRUE);
        WDT_EnableInt(WDT0);
        break;
