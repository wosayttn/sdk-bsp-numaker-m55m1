...
typedef struct
{
    ...
    /**
    * @var CANFD_T::DBTP
    * Offset: 0x0C  Data Bit Timing & Prescaler Register Register
    * ---------------------------------------------------------------------------------------------------
    * |Bits    |Field     |Descriptions
    * | :----: | :----:   | :---- |
    * |7[3:0]  |DSJW      |Data (Re) Synchronization Jump Width
    ...
    * |        |          |The bit time is built up from a multiple of this quanta. Valid values for the Bit Rate Prescaler are 0 to 31. When TDC ='1',the range is limited to 0,1.
    * |        |          |The actual interpretation by the hardware of this value is such that one more than the value programmed here is used.
    ...
    * @var CANFD_T::RWD
    * Offset: 0x14  RAM Watchdog Register
    * ---------------------------------------------------------------------------------------------------
    * |Bits    |Field     |Descriptions
    * | :----: | :----:   | :---- |
    * |[7:0]   |WDC       |Watchdog Conguration
    ...
    * |[1]     |CCE       |Conguration Change Enable
    * |        |          |0 = The CPU has no write access to the protected conguration registers.
    * |        |          |1 = The CPU has write access to the protected conguration registers (while CANFD_INIT (CANFD_CCCR[0]) = 1).
    * |[2]     |ASM       |Restricted Operation Mode
    * |        |          |Bit ASM can only be set by the Host when both CCE and INIT are set to 1.
    * |        |          |The bit can be reset by the Host software at any time.
    ...
    * |        |          |1 = The Controller may be set in power down by stopping AHB clock and CAN Core clockcclk.
    ...
    * |        |          |0x000-0x1FF The value by which the oscillator frequency is divided for generating the bit time quanta.
    ...
    * |        |          |Valid values are 0 to 127,Should be smaller than NTSEG2.
    * |        |          |The actual interpretation by the hardware of this value is such that one more than the value programmed here is used.
    * |        |          |tSJW = (NSJW + 1) x tq.
    * @var CANFD_T::TSCC
    * Offset: 0x20  Timestamp Counter Conufiguration
    * ---------------------------------------------------------------------------------------------------
    * |Bits    |Field     |Descriptions
    * | :----: | :----:   | :---- |
    ...
    * |        |          |A wrap around sets interrupt ag CANFD_IR (CANFD_IR[16])Write access resets the counter to 0.
    ...
    * @var CANFD_T::TOCC
    * Offset: 0x28  Timeout Counter Conufiguration
    * ---------------------------------------------------------------------------------------------------
    * |Bits    |Field     |Descriptions
    * | :----: | :----:   | :---- |
    ...
    * |        |          |0x00-0x7F Defines the minimum value for the SSP position, dominant edges on CANx_RXD that would result in an earlier SSP position are ignored for transmitter delay measurement
    ...
    * |        |          |0x00-0x7F Offset value defining the distance between the measured delay from CANx_TXD to CANx_RXD and the secondary sample point.
    * |        |          |Valid values are 0 to 127 mtq.
    * @var CANFD_T::IR
    ...
    * |        |          |Note: These are protected write bits, write access is possible only when bit CCE and bit INIT of CANFD_CCCR register are set to 1.
    * @var CANFD_T::HPMS
    ...
    * |        |          |Index of matching filter element. Range is 0 to CANFD_SIDFC.LSS - 1 respor. CANFD_XIDFC.LSE - 1.
    * |[15]    |FLST      |Filter List
    ...
    * |        |          |Note: In case the data field size of an accepted CAN frame exceeds the data field size configured for the matching Rx Buffer or Rx FIFO, only the number of bytes as configured by CANFD_RXESC are stored to the Rx Buffer resp Rx FIFO element.
    ...
    * |        |          |Note: Be aware that tThe sum of TFQS and NDTB may be not greater than 32.
    ...
    * |        |          |Read as zero when Tx Queue operation is configured (TFQM (CANFD_TXBC[3]) = 1).
    ...
    * |        |          |Note: If an add request is applied for a Tx Buffer with pending transmission request (corre- sponding CANFD_TXBRP bit already set), this add request is ignored.
    * @var CANFD_T::TXBCR
    ...
    */
    ...
    __IO uint32_t RXF0S;                /*!< [0x00a4] Rx FIFO 0 Status                                                 */
    ...
    __IO uint32_t RXF1S;                /*!< [0x00b4] Rx FIFO 1 Status                                                 */
    ...
    __IO uint32_t TXFQS;                /*!< [0x00c4] Tx FIFO/Queue Status                                             */
    __IO uint32_t TXESC;                /*!< [0x00c8] Tx Buffer Element Size Configuration                             */
    __IO uint32_t TXBRP;                /*!< [0x00cc] Tx Buffer Request Pending                                        */
    __IO uint32_t TXBAR;                /*!< [0x00d0] Tx Buffer Add Request                                            */
    __IO uint32_t TXBCR;                /*!< [0x00d4] Tx Buffer Cancellation Request                                   */
    __IO uint32_t TXBTO;                /*!< [0x00d8] Tx Buffer Transmission Occurred                                  */
    __IO uint32_t TXBCF;                /*!< [0x00dc] Tx Buffer Cancellation Finished                                  */
...
#define CANFD_CCCR_UTSU_Pos              (10)                                               /*!< CANFD_T::CCCR: UTSU Position           */
#define CANFD_CCCR_UTSU_Msk              (0x1ul << CANFD_CCCR_UTSU_Pos)                     /*!< CANFD_T::CCCR: UTSU Mask           */
