...
{
#if defined(BSP_USING_BPWM0)
    { .name = "bpwm0", .base = BPWM0, .rstidx = SYS_BPWM0RST, .modid = BPWM0_MODULE },
#endif

#if defined(BSP_USING_BPWM1)
    { .name = "bpwm1", .base = BPWM1, .rstidx = SYS_BPWM1RST, .modid = BPWM1_MODULE },
#endif
}; /* bpwm nu_bpwm */
...
#endif //#if defined(BSP_USING_BPWM) && defined(RT_USING_PWM)
