...
#define IRQ_MAX_NUM         32            //Max support 32
...
#define MAKE_GP_ISR(x, y) \
    void G##x##_IRQHandler(void) \
    { \
        rt_uint32_t u32IES; \
        \
        rt_interrupt_enter(); \
        u32IES = x->INTSRC; \
        pin_irq_hdr(u32IES, y); \
        x->INTSRC = u32IES; \
        rt_interrupt_leave(); \
    }

#define MAKE_GPI_ISR(x, y) \
    void x##_IRQHandler(void) \
    { \
        rt_uint32_t u32IES; \
        \
        rt_interrupt_enter(); \
...
MAKE_GP_ISR(PA, NU_PA)
MAKE_GP_ISR(PB, NU_PB)
MAKE_GP_ISR(PC, NU_PC)
MAKE_GP_ISR(PD, NU_PD)
MAKE_GP_ISR(PE, NU_PE)
MAKE_GP_ISR(PF, NU_PF)
MAKE_GP_ISR(PG, NU_PG)
MAKE_GP_ISR(PH, NU_PH)
MAKE_GPI_ISR(GPI, NU_PI)
MAKE_GP_ISR(PJ, NU_PJ)
