...
#ifndef __NU_SPIM_HYPER_H__
#define __NU_SPIM_HYPER_H__
...
/**
  ...
  * \hideinitializer
  */
#define SPIM_HYPER_ENABLE_HYPMODE(spim, x) (spim->CTL0 |= (x << SPIM_CTL0_DEVMODE_Pos))
 ...
 * \hideinitializer
 */
#define SPIM_HYPER_GET_DMM_IDLE(spim) ((spim->DMMCTL & SPIM_DMMCTL_DMMIDLE_Msk) >> SPIM_DMMCTL_DMMIDLE_Pos)
...
/*----------------------------------------------------------------------------*/
__STATIC_INLINE int32_t SPIM_HYPER_WaitDMMIdleState(SPIM_T *spim);
...
__STATIC_INLINE int32_t SPIM_HYPER_WaitDMMIdleState(SPIM_T *spim)
{
    ...
    /* HyperBus DMM Mode Done. */
    SPIM_HYPER_ENABLE_DMMDONE(spim);      
...
__STATIC_INLINE void SPIM_HYPER_DISABLE_CIPHER(SPIM_T *spim)
{
    SPIM_HYPER_WaitDMMIdleState(spim);
...
__STATIC_INLINE void SPIM_HYPER_DISABLE_CACHE(SPIM_T *spim)
{
    ...
    if (((spim->CTL0 & SPIM_CTL0_CIPHOFF_Msk) >> SPIM_CTL0_CIPHOFF_Pos) != SPIM_HYPER_OP_DISABLE)
    {
        SPIM_HYPER_SET_DMM_DESELTIM(spim, 0x08);
    }
}
...
__STATIC_INLINE void SPIM_HYPER_ENABLE_CACHE(SPIM_T *spim)
{
    ...
    /* Cipher Disabled Set Deselect Time 0x08 */
    if (((spim->CTL0 & SPIM_CTL0_CIPHOFF_Msk) >> SPIM_CTL0_CIPHOFF_Pos) != SPIM_HYPER_OP_DISABLE)
    {
        SPIM_HYPER_SET_DMM_DESELTIM(spim, 0x08);
    }
