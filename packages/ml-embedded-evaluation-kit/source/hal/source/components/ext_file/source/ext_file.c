...
#define DBG_SECTION_NAME  "ml.ext"
#define DBG_COLOR
#include <rtdbg.h>
...
int ext_file_list(const char *pcDirPath)
{
    ...
    if (!pcDirPath)
        goto exit_ext_file_list;

    psDIR = opendir(pcDirPath);
    if (!psDIR)
        goto exit_ext_file_list;

    while ((dp = readdir(psDIR)) != NULL)
    {
        LOG_I("%s/%s", pcDirPath, dp->d_name);
        n++;
    }
...
int ext_file_import(const char *pcPath, void **ppvBufAddr, uint32_t *pu32BufSize)
{
    ...
    rt_memset(&sb, 0, sizeof(struct stat));
    ...
    pvBufAddr = memheap_helper_allocate(evAREANA_AT_HYPERRAM, sb.st_size);
    if (pvBufAddr == RT_NULL)
    {
        LOG_E("Failed to allocate memory size (%dB).", sb.st_size);
        goto exit_ext_file_import;
    }

    if (((fd = open(pcPath, O_RDONLY, 0)) < 0) ||
            (sb.st_size != read(fd, pvBufAddr, sb.st_size)))
    {
    ...
    if (ppvBufAddr)
        *ppvBufAddr = pvBufAddr;

    if (pu32BufSize)
        *pu32BufSize = sb.st_size;

    ret = 0;

exit_ext_file_import:
